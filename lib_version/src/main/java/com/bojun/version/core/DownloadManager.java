...
import com.bojun.version.callback.DownloadListener;
import com.bojun.version.core.http.AllenHttp;
import com.bojun.version.core.http.FileCallBack;
...
public class DownloadManager {
    ...
    public static void downloadAPK(final String url, final VersionParams versionParams, final DownloadListener listener) {
        ...
        String downloadPath = versionParams.getDownloadAPKPath() + AllenChecker.getGlobalContext().getString(R.string.lib_download_apkname, AllenChecker.getGlobalContext().getPackageName());
        //静默下载也判断本地是否有缓存
        ...
        AllenHttp.getHttpClient().newCall(request).enqueue(new FileCallBack(versionParams.getDownloadAPKPath(), AllenChecker.getGlobalContext().getString(R.string.lib_download_apkname, AllenChecker.getGlobalContext().getPackageName())) {
            @Override
            public void onSuccess(File file, Call call, Response response) {
                ...
                if (versionParams.isShowNotification()) {
                    ...
                    finalBuilder.setContentText(AllenChecker.getGlobalContext().getString(R.string.lib_download_finish));
            ...
            public void onDownloading(int progress) {
                ...
                if (currentProgress - lastProgress >= 5) {
                    ...
                    if (versionParams.isShowNotification() && !isDownloadSuccess) {
                        ...
                        finalBuilder.setContentText(String.format(AllenChecker.getGlobalContext().getString(R.string.lib_download_progress), lastProgress));
            ...
            public void onDownloadFailed() {
                if (versionParams.isShowNotification()) {
                    ...
                    finalBuilder.setContentIntent(pendingIntent);
                    finalBuilder.setContentText(AllenChecker.getGlobalContext().getString(R.string.lib_download_fail));
    ...
    private static void silentDownloadAPK(final Context context, String url, final VersionParams versionParams, final DownloadListener listener) {
        ...
        if (listener != null)
            listener.onCheckerStartDownload();
        AllenHttp.getHttpClient().newCall(request).enqueue(new FileCallBack(versionParams.getDownloadAPKPath(), context.getString(R.string.lib_download_apkname, context.getPackageName())) {
            @Override
            public void onSuccess(File file, Call call, Response response) {
                listener.onCheckerDownloadSuccess(file);
            }
    ...
    private static NotificationCompat.Builder createNotification(Context context) {
        ...
        builder.setContentTitle(context.getString(R.string.lib_downloading));
        builder.setTicker(context.getString(R.string.lib_downloading));
        builder.setContentText(String.format(context.getString(R.string.lib_download_progress), 0));
